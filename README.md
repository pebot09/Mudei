...
| **Compras** | A planilha, turbinada: itens por categoria com prioridade, condição (novo/usado/doação), orçamento, melhor preço, preço pago, medidas do espaço, links de lojas, doador e responsável. Doações têm fluxo próprio (pedir → **🤝 prometido** → ganho) e não pesam na projeção de gastos; compras pagas por outra pessoa (ex.: seus pais) ficam fora do "seu bolso" e do teto. Busca, filtros, ordenação, "copiar lista de pendências" para o WhatsApp e **kit enxoval** — sugestões de ~50 itens para adicionar com um toque |
